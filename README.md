...
한국어 정치 토론 플랫폼. 토론 주제별 *좌우 분할(찬/반)* 영구 보관 + AI 의견정리 + 블라인드 답변 자기 거울.
...
- [x] Phase 2 — 토론 주제 색인 + 게시판 읽기 전용 (시드 14토론 주제 / 65의견)
- [x] Phase 3 — 의견 + 동조 + 댓글(무한 깊이) + 인용 의견 + 출처 반박
...
- 토론 주제 색인이 14개 표시되는가
- 게시판 상세에 의견들이 PRO/CON 으로 분할되어 보이는가
- 카카오로 로그인하면 닉네임 설정 화면으로 가는가
- 닉네임 설정 후 / 로 돌아오는가
- 의견 작성·동조·댓글·인용·반박이 모두 작동하는가
...
1. AI 는 사서지 판사가 아님 (있는 그대로 의견정리, 우열 판정 X)
2. 점수 합산 거부 (비추천 X, 게이미피케이션 X)
3. Private-by-Design (4축 가치관·블라인드는 본인만)
4. 4 대 함정 회피 (개인화·게이미피케이션·알림 폭격·광고)

## 다음 (Sprint 2)

`CLAUDE.md §6 Phase 4~7` — 토론 주제 만들기(AI 정제) · 사상검증(12 Likert) · 오늘의 3문항(블라인드) · 내 정보 자기 거울. AI 통합은 초기에 *Max 요금제 안 `claude -p` 헤드리스* 로 시작, 나중에 Anthropic API + OpenAI + Gemini 3개 fallback router 로 확장.
